if game.CoreGui:FindFirstChild("d3xteam.lol1") then
    game.CoreGui.d3xteam.lol1:Destroy()
end
...
local GenerateGUID = d3xteam.lol_Main.HttpService:GenerateGUID(false) 
...
function d3xteam.lol_Main:PlaySound(SoundID)
...
local d3xteam.lol = Instance.new("ScreenGui")
d3xteam.lol.Name = "d3xteam.lol1"
d3xteam.lol.Parent = game.CoreGui
d3xteam.lol.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

-- -- this function converts a string to base64
-- function d3xteam.lol_Main:encode(data)
...
-- function d3xteam.lol_Main:decode(data)
--     local b = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
...
function d3xteam.lol_Main:encode(data)
    ...
    encodedData = "-- d3xteam.lol by Mana64 \n\nreturn function() "..insertRandomWords2(encodedData)..' end) \n\n\n'

    return encodedData
end

function d3xteam.lol_Main:decode(data)
    data = data:gsub('^-- d3xteam.lol by Mana64 %s+return%s+function%(%) (.*) end%)%s*\n*$', '%1')
...
function d3xteam.lol_Main:GetDistance(player_pos, Endpoint)
...
function d3xteam.lol_Main:GetPlayerImage(ID)
    local width = 420
    local height = 420
    local format = "png"
...
function d3xteam.lol_Main:SetCFG(name)
    d3xteam.lol_Main.Targeted_Config = name
end

function d3xteam.lol_Main:LoadSettings(Folder, CFGName)

    local Encoded = readfile(Folder .. "/settings.txt")
    local Decoded = d3xteam.lol_Main:decode(Encoded)

    writefile(Folder .. "/settings.txt", tostring(d3xteam.lol_Main.HttpService:JSONEncode(Decoded)))

    d3xteam.lol_Main.Settings = d3xteam.lol_Main.HttpService:JSONDecode(readfile(Folder .. "/settings.txt"))
end

function d3xteam.lol_Main:AutoJoinDiscord(DiscordCode)
    ...
    if req then
        req({
            ...
            Body = d3xteam.lol_Main.HttpService:JSONEncode({
                cmd = 'INVITE_BROWSER',
                nonce = d3xteam.lol_Main.HttpService:GenerateGUID(false),
...
function ChangeTypeText(object)
    ...
    {TextColor3 = d3xteam.lol_Main.Theme.Custom.Text}
):Play()
end
...
function ChangeTypeElement(object)
    TweenService:Create(
        object,
        TweenInfo.new(.3, Enum.EasingStyle.Quad),
        {BackgroundColor3 = d3xteam.lol_Main.Theme.Custom.Element}
    ):Play()
end

function d3xteam.lol_Main:Window(config)
    ...
    function d3xteam.lol_Main:GetConfigNames()
        local ReturnTable = {}
    ...
    if not isfile(Folder .. "/settings.txt") then
        local content = {}
        for i,v in pairs(d3xteam.lol_Main.Settings) do
            content[i] = v
        end
        writefile(Folder .. "/settings.txt", tostring(d3xteam.lol_Main.HttpService:JSONEncode(content)))
    end
    d3xteam.lol_Main.Settings = d3xteam.lol_Main.HttpService:JSONDecode(readfile(Folder .. "/settings.txt"))
    ...
    function SaveSettings(bool)
        local rd = d3xteam.lol_Main.HttpService:JSONDecode(readfile(Folder.."/settings.txt"))
        state = bool
        if state then
            return rd
        end
        ...
        -- writefile(Folder .. "/settings.txt", tostring(d3xteam.lol_Main.HttpService:JSONEncode(d3xteam.lol_Main:encode(content))))
        writefile(Folder .. "/settings.txt", tostring(d3xteam.lol_Main.HttpService:JSONEncode(content)))
    end
    ...
    function SaveSettingsCFG(cfg)
        local content = {}
        for i, v in pairs(d3xteam.lol_Main.SettingsFlags) do
            content[i] = v.Value
        end
    
        local Encoded = d3xteam.lol_Main.HttpService:JSONEncode(content) -- Use HttpService
    ...
    function LoadSettingsCFG(cfg)
        ...
        local JSONData = d3xteam.lol_Main.HttpService:JSONDecode(Encoded) -- Use HttpService
    
        for a, b in pairs(JSONData) do
            if d3xteam.lol_Main.SettingsFlags[a] then
                spawn(function()
                    d3xteam.lol_Main.SettingsFlags[a]:Set(b)
                end)
            else
                warn("Error ", a, b)
            end
    ...
    function EditSettingsCFG(cfg, Name, newvalue)
        ...
        if d3xteam.lol_Main.SettingsFlags[Name] then
            spawn(function()
                d3xteam.lol_Main.SettingsFlags[Name]:Set(newvalue)
            end)
        end
    end
    ...
    function SystemT(title, callback)
        ...
        SetupSystemToggle.MouseButton1Click:Connect(function()
            d3xteam.lol_Main:PlaySound(d3xteam.lol_Main.Lib_Sounds.ClickSound)
            SToggled = not SToggled
            SystemTogglefunc:Set(SToggled)
        end)

        d3xteam.lol_Main.SettingsFlags[title] = SystemTogglefunc
    ...
    spawn(function()
        while wait() do
            if RememberKey == false and HasBeenToggled == false then
                pcall(function()
                    EditSettingsCFG("Keyd3xteam.lol", "Key Holder", "")
                end)
            end
        end
    end)
    ...
    function SystemK(title, callback)
        ...
        d3xteam.lol_Main.SettingsFlags[title] = KeyBoxfunc
        return KeyBoxfunc
    end
    ...
    LoadSettingsCFG("Keyd3xteam.lol")
    ...
    LoadButton.MouseButton1Click:Connect(function()
        ...
        if table.find(KeyAccess, KeyHolderText) then
            SaveSettingsCFG("Keyd3xteam.lol")
    ...
    function d3xteam.lol_Main:VisMain(v)
        MainFrame.Visible = v
    end
    ...
    PlayerImage.Image = d3xteam.lol_Main:GetPlayerImage(Player.UserId)
    ...
    ToggledFrame.Parent = d3xteam.lol
    ...
    NotifyHolder.Parent = d3xteam.lol
    ...
    function d3xteam.lol_Main:Notify(cfg)
    ...
    --[[
        ...
        for i,v in pairs(ListScripts) do
            ...
            ShareScript.MouseEnter:Connect(function()
                d3xteam.lol_Main:PlaySound(d3xteam.lol_Main.Lib_Sounds.HoverSound)
            end)
        end
        ...
        --             Method = "GET"
        --         })
        --         local data = d3xteam.lol_Main.HttpService:JSONDecode(data.Body)
        ...
        --                 ChatSocketFrame.Parent = ChatFrameFrame --game:GetService("CoreGui").d3xteam.lol1.MainFrame.ChatFrame.ChatFrameFrame
        ...
        ChatBoxText.FocusLost:Connect(function(ep)
            if ep then
                local Data = d3xteam.lol_Main.HttpService:JSONEncode({
        ...
        spawn(function()task.wait(.5)d3xteam.lol_Main:AutoJoinDiscord("qq6WgyMwkw")end)
    ...
    function TabsSec:TSection(title)
        ...
        -- function d3xteam.lol_Main:LoadCfg(cfg)
        --     local Encoded = readfile(Folder1 .. "/configs/" .. cfg .. ".txt")
        --     local Decoded = d3xteam.lol_Main:decode(Encoded)
            
        --     local Encode = d3xteam.lol_Main.HttpService:JSONEncode(Decoded)
        ...
        --         print(a,b)
        --     if d3xteam.lol_Main.Flags[a] then
        --         spawn(function()
        --           d3xteam.lol_Main.Flags[a]:Set(b)
        --        end)
        --     else
        --         warn("Error ", a,b)
        --     end
        ...
        -- function d3xteam.lol_Main:SaveCfg(cfg)
        --     local content = {}
        --     for i,v in pairs(d3xteam.lol_Main.Flags) do
        --         content[i] = v.Value
        --     end
        --     writefile(Folder1.."/configs/"..cfg..".txt", d3xteam.lol_Main:encode(tostring(d3xteam.lol_Main.HttpService:JSONEncode(content)))) -- FolderName.."/configs/"..name..".cfg"
        -- end
      
        -- function d3xteam.lol_Main:CreateCfg(cfg)
        --     local content = {}
        --     for i,v in pairs(d3xteam.lol_Main.Flags) do
        --         content[i] = v.Value
        --     end
        --     writefile(Folder1.."/configs/"..cfg..".txt", d3xteam.lol_Main:encode(d3xteam.lol_Main.HttpService:JSONEncode(content))) -- FolderName.."/configs/"..name..".cfg"
        --     -- writefile("d3xteam.lol/configs/Mana64.txt", d3xteam.lol_Main:encode(tostring(content)))
        -- end

        function d3xteam.lol_Main:LoadCfg(cfg)
            local Encoded = readfile(Folder1 .. "/configs/" .. cfg .. ".txt")

            local JSONData = d3xteam.lol_Main.HttpService:JSONDecode(d3xteam.lol_Main:decode(Encoded))
        ...
        function d3xteam.lol_Main:SaveCfg(cfg)
            ...
            local Encoded = d3xteam.lol_Main:encode(d3xteam.lol_Main.HttpService:JSONEncode(content))
            
            writefile(Folder1 .. "/configs/" .. cfg .. ".txt", Encoded)
        end

        function d3xteam.lol_Main:Edit_LastLoad(cfg)
            writefile(Folder1.."/LastLoaded.txt", d3xteam.lol_Main.HttpService:JSONEncode({["CFG"] = tostring(cfg)}))
        end

        function d3xteam.lol_Main:LastConfigSaved()
            if isfile(Folder1.."/LastLoaded.txt") then
                return d3xteam.lol_Main.HttpService:JSONDecode(readfile(Folder1.."/LastLoaded.txt")).CFG
            else
                d3xteam.lol_Main:Notify({Title = "d3xteam.lol",
        ...
        function d3xteam.lol_Main:CreateCfg(cfg)
            ...
            local Encoded = d3xteam.lol_Main.HttpService:JSONEncode(content) -- Convert to JSON string
            
            writefile(Folder1 .. "/configs/" .. cfg .. ".txt", Encoded)
        end
        
        SaveCFGB.MouseButton1Click:Connect(function()
            if d3xteam.lol_Main.Targeted_Config == "" then
                d3xteam.lol_Main:Notify({Title = "d3xteam.lol",
                ...
                d3xteam.lol_Main:Notify({Title = "d3xteam.lol",
                    Text = "Saved to: "..tostring(d3xteam.lol_Main.Targeted_Config),
                    Time = 2,
                    AutoClose = true
                })
                d3xteam.lol_Main:SaveCfg(tostring(d3xteam.lol_Main.Targeted_Config))
                Edit_LastLoad(tostring(d3xteam.lol_Main.Targeted_Config))
            end
        end)
        ...
        function Tabs:Tab(title)
            ...
            function Sections:Section(title)
                    ...
                    while task.wait() do
                        pcall(function()
                            TweenService:Create(
                                Section,
                                TweenInfo.new(.3, Enum.EasingStyle.Quad),
                                {BackgroundColor3 = d3xteam.lol_Main.Theme.Custom.Section}
                            ):Play()
                        end)
                    end
                end)
                    ...
                    spawn(function()
                        while task.wait() do
                            pcall(function()
                                ...
                                ButtonStroke.Color = d3xteam.lol_Main.Theme.Custom.Element
                            end)
                        end
                    end)
                ...
                function Elements:Toggle(title, callback)
                    ...
                    d3xteam.lol_Main.Flags[title] = Togglefunc
                    return Togglefunc
                end
                ...
                function Elements:Dropdown(title, list, callback)
                    ...
                    spawn(function()
                        while task.wait() do
                            pcall(function()
                                ...
                                Arrow.ImageColor3 = d3xteam.lol_Main.Theme.Custom.Element
                            end)
                        end
                    end)
                    ...
                    d3xteam.lol_Main.Flags[title] = Dropfunc
                    return Dropfunc
                end
                ...
                function Elements:Slider(title, min, max, start, callback)
                    ...
                    d3xteam.lol_Main.Flags[title] = Sliderfunc
                    return Sliderfunc
                end
                ...
                function Elements:Colorpicker(title, preset, callback)
                        ...
                        while task.wait() do
                            pcall(function()
                                TweenService:Create(
                                    ColorPFrameGlow,
                                    TweenInfo.new(.4, Enum.EasingStyle.Quad),
                                    {ImageColor3 = d3xteam.lol_Main.Theme.Custom.Glow}
                                ):Play()
                            end)
                        end
                    end)
                        ...
                        while task.wait() do
                            pcall(function()
                                TweenService:Create(
                                    ColorpreviewStroke,
                                    TweenInfo.new(.3, Enum.EasingStyle.Quad),
                                    {Color = d3xteam.lol_Main.Theme.Custom.Element}
                                ):Play()
                           ...
                           if input.UserInputType == Enum.UserInputType.MouseButton1 then
                              ...
                              ColorInput =
                                 RunService.RenderStepped:Connect(
                                    function()
                                    local ColorX =
                                       (math.clamp(d3xteam.lol_Main.Mouse.X - ChoseColor.AbsolutePosition.X, 0, ChoseColor.AbsoluteSize.X) /
                                       ChoseColor.AbsoluteSize.X)
                                    local ColorY =
                                       (math.clamp(d3xteam.lol_Main.Mouse.Y - ChoseColor.AbsolutePosition.Y, 0, ChoseColor.AbsoluteSize.Y) /
                                       ChoseColor.AbsoluteSize.Y)
                           ...
                           if input.UserInputType == Enum.UserInputType.MouseButton1 then
                              ...
                              HueInput =
                                 RunService.RenderStepped:Connect(
                                    function()
                                    local HueY =
                                       (math.clamp(d3xteam.lol_Main.Mouse.Y - Hue.AbsolutePosition.Y, 0, Hue.AbsoluteSize.Y) /
                                          Hue.AbsoluteSize.Y)
                    ...
                    d3xteam.lol_Main.Flags[title] = Colorpickerfunc
                ...
                function Elements:Bind(title, callback, external)
                    ...
                    AddBind.MouseButton1Click:Connect(function()
                        ...
                        TweenService:Create(
                            BindSettingImage,
                            TweenInfo.new(.3, Enum.EasingStyle.Quad),
                            {ImageColor3 = d3xteam.lol_Main.Theme.Custom.Element}
                        ):Play()
    ...
    spawn(function()
        ...
        Sec2:Button("Create Config", function()
            d3xteam.lol_Main:CreateCfg(tostring(Config_Name))
            d3xteam.lol_Main:Notify({
                Title = "d3xteam.lol",
        ...
        local Configs_Drop = Sec1:Dropdown("Select Config", d3xteam.lol_Main:GetConfigNames(), function(t)
            Selected_Config = t
            d3xteam.lol_Main:Notify({Title = "d3xteam.lol",
                Text = "Targeted CFG: "..tostring(Selected_Config)
            })
            d3xteam.lol_Main:SetCFG(tostring(Selected_Config))
        end)

        Sec1:Button("Refresh Configs", function()
            Configs_Drop:Refresh(d3xteam.lol_Main:GetConfigNames())
        end)
        Sec1:Line()
        Sec1:Button("Load Selected Config", function()
            ...
            d3xteam.lol_Main:LoadCfg(tostring(Selected_Config))
            d3xteam.lol_Main:Edit_LastLoad(tostring(Selected_Config))
        end)
        Sec3:Colorpicker("Background", d3xteam.lol_Main.Theme.Custom.Background, function(t)
            d3xteam.lol_Main.Theme.Custom.Background = t
        end)
        Sec3:Colorpicker("Section", d3xteam.lol_Main.Theme.Custom.Section, function(t)
            d3xteam.lol_Main.Theme.Custom.Section = t
        end)
        Sec3:Colorpicker("Element", d3xteam.lol_Main.Theme.Custom.Element, function(t)
            d3xteam.lol_Main.Theme.Custom.Element = t
        end)
        Sec3:Colorpicker("Text", d3xteam.lol_Main.Theme.Custom.Text, function(t)
            d3xteam.lol_Main.Theme.Custom.Text = t
        end)
        Sec3:Colorpicker("Glow", d3xteam.lol_Main.Theme.Custom.Glow, function(t)
            d3xteam.lol_Main.Theme.Custom.Glow = t
        end)
        ...
        if not isfile(Folder..'/On_Launch.json') then
            writefile(Folder..'/On_Launch.json', d3xteam.lol_Main.HttpService:JSONEncode({
                ["On_Launch"] = false,
            }))
        end
        ...
        local Get_Load = d3xteam.lol_Main.HttpService:JSONDecode(readfile(Folder..'/On_Launch.json')).On_Launch

        Get_ALC:Set(Get_Load)

        game.CoreGui:WaitForChild("d3xteam.lol1").MainFrame.Visible = Get_Load
    ...
    spawn(function()
        while task.wait() do
            pcall(function()
                --// Background \\--
                KeyFrame.BackgroundColor3 = d3xteam.lol_Main.Theme.Custom.Background
                MainFrame.BackgroundColor3 = d3xteam.lol_Main.Theme.Custom.Background
                LeftFrame.BackgroundColor3 = d3xteam.lol_Main.Theme.Custom.Background
                ...
                --// Element \\
                -- d3xteam.lol_Main.Theme.Custom.Section
                -- d3xteam.lol_Main.Theme.Custom.Element
                -- d3xteam.lol_Main.Theme.Custom.Text
            end)
        end
    end)
    return TabsSec
